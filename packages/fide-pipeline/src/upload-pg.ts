...
export async function ensureSchema(): Promise<void> {
  ...
  // Idempotent migration: online players, games, links, dedup tables
  const { rows: onlineCheck } = await sql`
    SELECT COUNT(*)::int AS count
    FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'online_players'
  `;
  if ((onlineCheck[0].count as number) === 0) {
    console.log("  Creating online profile tables...");
    await sql`
    ...
    `;
    await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_online_players_platform_id ON online_players (platform, platform_id)`;
    await sql`
    ...
    `;
    await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_online_games_dedup ON online_games (platform, platform_game_id, online_player_id)`;
    await sql`CREATE INDEX IF NOT EXISTS idx_online_games_player_date ON online_games (online_player_id, played_at DESC)`;
    await sql`
    ...
    `;
    await sql`CREATE INDEX IF NOT EXISTS idx_links_approved ON online_player_links (player_id) WHERE status = 'approved'`;
    await sql`CREATE INDEX IF NOT EXISTS idx_links_pending ON online_player_links (status) WHERE status = 'pending'`;
    await sql`
    ...
    `;
    console.log("  Online profile tables created.");
  }
}
